package com.basic.advance.advanced_class_06;
...
public class Code_01_CoinsWay {
	...
	//index可以任意自由使用index及其以后所有的钱   aim 目标钱数
	public static int process1(int[] arr, int index, int aim) {
		int res = 0;
		if (index == arr.length) {
			res = aim == 0 ? 1 : 0;
		} else {
			// i 张数 index+1 当前货币已经做出选择
...
//  key  index_aim  value  的返回值是固定的
	public static HashMap<String,Integer> map =new HashMap<>();


	//使用map记录之前已经统计过得 钱所能组合出的数
	public static int process_map(int[] arr, int index, int aim) {
		...
		//相当于一个缓存，记忆化搜索
		map.put(String.valueOf(index)+"_"+String.valueOf(aim),res);  //返回之前将相应的map 记录下来
		return res;
	}
